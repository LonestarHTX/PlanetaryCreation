...
bool FTerraneTransportTest::RunTest(const FString& Parameters)
{
    ...
    UE_LOG(LogTemp, Log, TEXT("=== Milestone 6 Task 1.2: Terrane Transport Test ==="));
    UE_LOG(LogTemp, Log, TEXT(""));
    ...
    UE_LOG(LogTemp, Log, TEXT("Plate configuration: %d oceanic, %d continental"), OceanicCount, ContinentalCount);

    // ========================================
    // TEST 1: Extract Terrane and Verify Carrier Assignment
    // ========================================
    UE_LOG(LogTemp, Log, TEXT(""));
    UE_LOG(LogTemp, Log, TEXT("--- Test 1: Carrier Assignment ---"));
    ...
    if (Terranes.Num() > 0)
    {
        ...
        if (CarrierPlate)
        {
            ...
            UE_LOG(LogTemp, Log, TEXT("  Carrier plate %d assigned (oceanic)"), Terrane.CarrierPlateID);
        }
    }

    UE_LOG(LogTemp, Log, TEXT("  ✅ PASS: Carrier assignment successful"));
    UE_LOG(LogTemp, Log, TEXT(""));

    // ========================================
    // TEST 2: Terrane Migration Tracking
    // ========================================
    UE_LOG(LogTemp, Log, TEXT("--- Test 2: Terrane Migration Tracking ---"));

    // Capture initial terrane centroid
    const FVector3d InitialCentroid = Terranes[0].Centroid;
    UE_LOG(LogTemp, Log, TEXT("  Initial centroid: (%.4f, %.4f, %.4f)"), InitialCentroid.X, InitialCentroid.Y, InitialCentroid.Z);
    ...
    if (TerranesAfterMigration.Num() > 0)
    {
        ...
        UE_LOG(LogTemp, Log, TEXT("  Final centroid: (%.4f, %.4f, %.4f)"), FinalCentroid.X, FinalCentroid.Y, FinalCentroid.Z);
        ...
        UE_LOG(LogTemp, Log, TEXT("  Distance moved: %.2f km over 20 My"), DistanceKm);
        UE_LOG(LogTemp, Log, TEXT("  Migration time: %.2f ms (10 steps)"), MigrationTimeMs);
        ...
        UE_LOG(LogTemp, Log, TEXT("  Per-step overhead: %.3f ms (target: <1ms)"), PerStepMs);
        TestTrue(TEXT("Migration performance <1ms per step"), PerStepMs < 1.0);
    }

    UE_LOG(LogTemp, Log, TEXT("  ✅ PASS: Terrane migration tracking working"));
    UE_LOG(LogTemp, Log, TEXT(""));

    // ========================================
    // TEST 3: State Transitions
    // ========================================
    UE_LOG(LogTemp, Log, TEXT("--- Test 3: State Transitions ---"));
    ...
    if (TerranesAfterMigration.Num() > 0)
    {
        ...
        UE_LOG(LogTemp, Log, TEXT("  Current state after 10 steps: %d (0=Attached, 1=Extracted, 2=Transporting, 3=Colliding)"),
        ...
        {
            UE_LOG(LogTemp, Log, TEXT("  ✅ Terrane detected collision within 10 steps (early collision detection working)"));
            TestTrue(TEXT("Target plate assigned for collision"), TerranesAfterMigration[0].TargetPlateID != INDEX_NONE);
        }
        else
        {
            UE_LOG(LogTemp, Log, TEXT("  ✅ Terrane still transporting (collision not yet detected)"));
        }
    }

    UE_LOG(LogTemp, Log, TEXT("  ✅ PASS: State transitions valid"));
    UE_LOG(LogTemp, Log, TEXT(""));

    // ========================================
    // Summary
    // ========================================
    UE_LOG(LogTemp, Log, TEXT("=== Terrane Transport Test Summary ==="));
    UE_LOG(LogTemp, Log, TEXT("  ✅ Carrier assignment: PASS"));
    UE_LOG(LogTemp, Log, TEXT("  ✅ Migration tracking: PASS"));
    UE_LOG(LogTemp, Log, TEXT("  ✅ State transitions: PASS"));
    UE_LOG(LogTemp, Log, TEXT("  ✅ Performance (<1ms/step): PASS"));
    UE_LOG(LogTemp, Log, TEXT(""));
    UE_LOG(LogTemp, Log, TEXT("Terrane Transport Test PASSED"));
