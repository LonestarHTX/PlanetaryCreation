...
#include "RealtimeMeshComponent/Public/Data/RealtimeMeshShared.h"
...
void FTectonicSimulationController::StepSimulation(int32 Steps)
{
    if (UTectonicSimulationService* Service = GetService())
    {
        ...
        EnsurePreviewActor();
        UpdatePreviewMesh(BuildPreviewMesh(6370.0f));
    }
}
...
FTectonicSimulationController::FPreviewMeshPayload FTectonicSimulationController::BuildPreviewMesh(float RadiusKm) const
{
    FPreviewMeshPayload Payload;
    ...
    TRealtimeMeshBuilderLocal<uint16, FPackedNormal, FVector2DHalf, 1> Builder(Payload.StreamSet);
    Builder.EnableTangents();
    Builder.EnableTexCoords();
    Builder.EnableColors();

    const float RadiusUnits = RadiusKm; // 1 Unreal unit == 1 km for editor tooling
    ...
    Payload.VertexCount = VertexIndices.Num();
...
void FTectonicSimulationController::UpdatePreviewMesh(FPreviewMeshPayload&& Payload)
{
    ...
    {
        Mesh->CreateSectionGroup(GroupKey, MoveTemp(Payload.StreamSet)).Wait();
        bPreviewInitialized = true;
    }
    else
    {
        Mesh->UpdateSectionGroup(GroupKey, MoveTemp(Payload.StreamSet)).Wait();
    }

    const FRealtimeMeshSectionKey SectionKey = FRealtimeMeshSectionKey::CreateForPolyGroup(GroupKey, 0);
    const int32 VertexCount = FMath::Max(Payload.VertexCount, 0);
    const int32 TriangleCount = FMath::Max(Payload.TriangleCount, 0);
