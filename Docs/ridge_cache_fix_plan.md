> **Status Update (2025-10-10b):** Stage B profiling now honors the command-line `-SetCVar="r.PlanetaryCreation.StageBProfiling=1"` flag; automation emits `[StageB][Profile]` and `[StageB][CacheProfile]` again. Use the refreshed tables in `Docs/Performance_M6.md` for the canonical cache timings (≈12.1 ms continental amplification + 6.9 ms cache rebuild steady state).
>
> **Status Update (2025-10-10a):** Continental exemplar caching now stores the blended atlas samples per Stage B serial, so CPU fallbacks reuse the first replay instead of re-sampling SRTM patches each time. Blend cache serials track the post-bump Stage B counter, so the reuse kicks in immediately on the next readback. Watch `[ContinentalGPUReadback] Overrides` in Stage B logs—after the initial pass it should stay quiet unless new exemplar indices are dirtied.
>
> **Status Update (2025-10-09):** Stage B now caches prior Voronoi assignments between builds, dirties only the reassigned vertices (~93 k steady state for L7), and copies live render geometry during fast preview refreshes so the viewport stays in sync with ridge metrics. Oceanic parity shows Ridge 0.00–0.04 ms with incremental counts logged per step. Keep `RidgeDirectionCacheTest` plus the Milestone 6 GPU parity suites on CI to guard the snapshot path.
>
